...
class GoogleMapPlotter(object):
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, title="no implementation", imgname=""):
        if c:
            color = c
    ...
    def write_points(self, f):
        ...
        f.write('\t\t\t\ttitle: locations[i][3],\n')
        f.write('\t\t\t\ticon: img,\n')
        f.write('\t\t\t\tmap: map\n')
        f.write('\t\t\t});\n')
        f.write("\t\t\tgoogle.maps.event.addListener(marker, 'click', (function(marker, i) {\n")
        f.write('\t\t\t\treturn function() {\n')
        f.write('\t\t\t\t\tif (locations[i][4] !== "") {\n')
        f.write('\t\t\t\t\t\tinfoWindow.setContent(\'<h4>\'+locations[i][3]+\'</h4>\'\n')
        f.write('\t\t\t\t\t\t+\'<div><img width=\"254\" height=\"254\" src=\"\'+locations[i][4]+\'\"</div>\');\n')
        f.write('\t\t\t\t\t} else {\n')
        f.write('\t\t\t\t\t\tinfoWindow.setContent(\'<h4>\'+locations[i][3]+\'</h4>\');\n')
        f.write('\t\t\t\t\t}\n')
